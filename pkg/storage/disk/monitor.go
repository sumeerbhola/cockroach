...
func (m *MonitorManager) CollectInstantaneous(buf []Stats, buf2 []byte) ([]Stats, []byte, error) {
	m.mu.Lock()
	disks := m.mu.disks
	...
	buf = buf[:0]
	if collector == nil {
		return buf, buf2, errors.Errorf("no disks are being monitored")
	}
	now := timeutil.Now()
	var err error
	var err2 error
	_, buf2, err2 = collector.collectInstantaneous(disks, now, func(e traceEvent) {
		if e.err != nil {
			err = e.err
			return
		}
