...
func CreateMVCCReadWriter(
	...
	if txn != nil {
		...
		intentIter = reader.NewIterator(intentOpts, storage.MVCCKeyAndIntentsIterKind)
	}
	mvccRW := &intentAwareReadWriter{
		...
		valueIter:        reader.NewIterator(opts, storage.MVCCKeyAndIntentsIterKind),
...
func (rw *intentAwareReadWriter) SeekGE(key roachpb.Key) {
	...
	if rw.txn != nil {
		...
		// It is odd to use an interface that expects an Key when we are
