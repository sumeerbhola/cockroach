...
// PutRaw implements the Engine interface.
func (p *Pebble) PutRaw(key StorageKey, value []byte) error {
	if len(key.key) == 0 {
		return emptyKeyError()
	}
...
func (p *pebbleReadOnly) PutRaw(key StorageKey, value []byte) error {
	panic("not implemented")
}
